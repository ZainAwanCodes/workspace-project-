...
export const Modal = ({ isOpen, onClose, title, children, className, size = 'md' }: ModalProps) => {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto overflow-x-hidden bg-gray-900/50 dark:bg-gray-900/80 backdrop-blur-sm p-4 sm:p-0">
      <div
        ref={modalRef}
        className={cn(
          'relative w-full rounded-xl bg-white dark:bg-gray-900 shadow-2xl flex flex-col',
          sizes[size],
          size === 'full' && 'm-4',
          className
        )}
